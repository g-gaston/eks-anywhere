...
type providerTemplateNameGenerator interface {
	envVarForTemplate(osFamilyStr, osVersionStr, eksDName string) string
	defaultNameForTemplate(osFamilyStr, osVersionStr, eksDName string) string
	searchTemplate(ctx context.Context, template string) (string, error)
	defaultEnvVarForTemplate(osFamilyStr, osVersionStr string, kubeVersion anywherev1.KubernetesVersion) string
}
...
func (tc *templateRegistry) templateForRelease(t *testing.T, osFamily anywherev1.OSFamily, release *releasev1.EksARelease, kubeVersion anywherev1.KubernetesVersion, osVersion ...string) string {
	t.Helper()
	osFamilyStr := string(osFamily)
	versionsBundle := readVersionsBundles(t, release, kubeVersion)
	eksDName := versionsBundle.EksD.Name

	// If an OS version is not passed in, then we use the default version for this OS family.
	// Otherwise we use the OS version passed in
	osVersionStr := defaultOSVersions[osFamilyStr]
	if len(osVersion) > 0 {
		osVersionStr = osVersion[0]
	}

	templateEnvVarName := tc.generator.envVarForTemplate(osFamilyStr, osVersionStr, eksDName)
	...
	template = tc.generator.defaultNameForTemplate(osFamilyStr, osVersionStr, eksDName)
	if template != "" {
	...
	templateEnvVarName = tc.generator.defaultEnvVarForTemplate(osFamilyStr, osVersionStr, kubeVersion)
	template, ok = os.LookupEnv(templateEnvVarName)
	if !ok || template == "" {
