...
const (
	...
	Ubuntu2004    OS = "ubuntu" // We don't add the version number in the string to facilitate backwards compatibility
	Bottlerocket1 OS = "br"     // We don't add the version number in the string to facilitate backwards compatibility
	RedHat8       OS = "redhat" // We don't add the version number in the string to facilitate backwards compatibility
)
...
func (tc *templateRegistry) templateForRelease(t *testing.T, operatingSystem OS, release *releasev1.EksARelease, kubeVersion anywherev1.KubernetesVersion) string {
	t.Helper()
