...
func TestClusterValidateCreateActiveFeatureWorkload(t *testing.T) {
	...
	g := NewWithT(t)
	cluster.SetManagedBy("management-cluster")
	validateCreateError := cluster.ValidateCreate()
	g.Expect(validateCreateError).NotTo(Succeed())
	g.Expect(cluster.ValidateCreate()).To(MatchError(ContainSubstring("cluster on existing cluster is not supported")))
}
...
func TestClusterValidateCreateInvalidCluster(t *testing.T) {
	...
	g := NewWithT(t)
	g.Expect(cluster.ValidateCreate()).NotTo(Succeed())
	strings.Contains(cluster.ValidateCreate().Error(), "control plane node count must be positive")
}
