...
loop:
	...
	// This is not necessary, since if we get here, all workers are done, regardless if all the jobs
	// were run or not, so canceling the context is not necessary since there nothing else using it
	// This is just to avoid a warning for possible context leeking
	abortRemainingJobs()
